...
- 🔁 接口重试：可配置错误码、次数与间隔，上游临时故障自动重试，429 欠费不重试
...
当上游 API 返回可重试的错误码时（如 500、503、504），网关会自动等待一段时间后重新发送同一请求，提高临时故障下的成功率。

### 配置方式

1. 打开设置页面，找到「接口重试」配置卡片
2. 启用重试开关
3. 填写可重试的状态码（逗号分隔，如 `400, 500, 503, 504`）
4. 设置最大重试次数（如 3 次）与重试间隔（如 5 秒或 10 秒）

### 规则说明

- **仅重试配置中列出的状态码**：`429`（欠费/限流）、`401`（鉴权失败）等无需重试的状态码不会触发重试
- **连接级错误也会重试**：上游连接被拒绝、超时、DNS 解析失败等网络异常同样纳入重试
- **重试只在响应尚未发给客户端时进行**：流式请求一旦开始向客户端输出内容，就不会中途重试，避免客户端收到重复数据
- **与故障转移（failover）协同**：同一提供商按配置重试次数耗尽后，才会切换到下一个提供商
- 重试成功的请求会在日志中显示紫色 `↻N` 标签（N 为重试次数）

### 托盘快捷开关

点击系统托盘图标打开面板，可快速开关接口重试，无需打开设置页面。切换后立即生效，无需重启代理。
